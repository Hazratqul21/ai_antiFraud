...
def get_stats(db: Session = Depends(get_db)):
    ...
    blocked_transactions = db.query(models.Transaction).filter(models.Transaction.status == "BLOCK").count()
    challenged_transactions = db.query(models.Transaction).filter(models.Transaction.status == "CHALLENGE").count()
    approved_transactions = db.query(models.Transaction).filter(models.Transaction.status == "ALLOW").count()
    pending_transactions = db.query(models.Transaction).filter(models.Transaction.status == "PENDING").count()
