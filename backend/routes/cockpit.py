...
@router.get("/alerts")
def get_alerts(status: str | None = None, limit: int = 50, db: Session = Depends(get_db)):
...
def assign_alert(alert_id: int, payload: dict, db: Session = Depends(get_db)):
...
def resolve_alert(alert_id: int, payload: dict, db: Session = Depends(get_db)):
...
@router.get("/cases")
def get_cases(analyst: str | None = None, limit: int = 20, db: Session = Depends(get_db)):
