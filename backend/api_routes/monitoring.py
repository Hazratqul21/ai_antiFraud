...
def get_model_performance(days: int = 7, db: Session = Depends(database.get_db)):
    """
...
def get_alerts(limit: int = 10, db: Session = Depends(database.get_db)):
    """
...
def get_analyst_decisions(days: int = 30, db: Session = Depends(database.get_db)):
    """
...
def submit_feedback(payload: dict, db: Session = Depends(database.get_db)):
    """
