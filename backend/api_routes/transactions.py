...
def approve_transaction(transaction_id: int, db: Session = Depends(database.get_db)):
    """
    Force-approve a transaction (used for manual review override)
    """
    return _update_transaction_status(transaction_id, models.TransactionStatus.APPROVED, db)


@router.post("/transactions/{transaction_id}/unblock", response_model=schemas.TransactionWithRisk)
def unblock_transaction(transaction_id: int, db: Session = Depends(database.get_db)):
    """
    Unblock a previously blocked transaction after manual verification
    """
    return _update_transaction_status(transaction_id, models.TransactionStatus.APPROVED, db, require_status=models.TransactionStatus.BLOCKED)
